...
Descriptor::parse_descriptor_file (string filename)
{
	...
	if (s_value.substr (0, 2) == "0x") {
	    ...
	    temp >> std::hex >> value;
	    info_msg ("Descriptor: Setting %s with value 0x%x",
		      field.c_str (), value);
	}
	else {
	...
	// reg0x30
	    ...
	    regs[to_vector_index (0x30)] |= (value & 0xFF) << 56;
	}
	else if (field == "PerProcessPSA_length") {
	    regs[to_vector_index (0x30)] =
		(regs[to_vector_index (0x30)] & 0xFF00000000000000);
	    regs[to_vector_index (0x30)] |= (value & 0xFFFFFFFFFFFFFF);
	}
	// reg0x38
...
uint64_t
     Descriptor::get_reg (uint32_t word_address) const
     {
