			    ...
			    uint8_t abort, uint16_t context)
{
	...
	debug_msg
	    ("OtherCommand: handle=0x%x address=0x%016lx code=0x%04x size=0x%02x",
	     context, address, Command::code, command_size);
...
void
OtherCommand::process_response (AFU_EVENT * afu_event, uint8_t *)
{
    if (Command::state == IDLE) {
	error_msg
			   ...
			   uint8_t abort, uint16_t context)
{
    ...
    debug_msg
	("LoadCommand: handle=0x%x address=0x%016lx code=0x%04x size=0x%02x",
	 context, address, Command::code, command_size);
...
void
LoadCommand::process_response (AFU_EVENT * afu_event, uint8_t * cache_line)
{
			    ...
			    uint8_t abort, uint16_t context)
{
    ...
    debug_msg
	("StoreCommand: handle=0x%x address=0x%016lx code=0x%04x size=0x%02x",
	 context, address, Command::code, command_size);
...
void
StoreCommand::process_response (AFU_EVENT * afu_event, uint8_t * cache_line)
{
