...
void
MachineController::send_command (AFU_EVENT * afu_event, uint32_t cycle)
{
    bool try_send = true;
...
MachineController::process_response (AFU_EVENT * afu_event, uint32_t cycle)
{
    ...
    debug_msg ("MachineController: Response code %d",
...
{
    uint32_t
	i = word_address / (SIZE_CONFIG_TABLE * 2);
    ...
    uint32_t
	offset = word_address % (SIZE_CONFIG_TABLE * 2);
