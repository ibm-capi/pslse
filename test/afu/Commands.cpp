...
#include "Commands.h"
 ...
 Command::Command(uint16_t c, bool comm_addr_par, bool comm_code_par, bool comm_tag_par, bool buff_read_par):code(c), completed(true), state(IDLE), command_address_parity(comm_addr_par),
command_code_parity(comm_code_par), command_tag_parity(comm_tag_par),
buffer_read_parity(buff_read_par)
{
}
...
void OtherCommand::send_command(AFU_EVENT * afu_event, uint32_t new_tag,
				...
				uint8_t abort, uint16_t context)
{
	...
	uint32_t tag_parity =
	    (command_tag_parity) ? generate_parity(new_tag + 1,
						   ODD_PARITY) :
	    generate_parity(new_tag, ODD_PARITY);
	uint32_t code_parity =
	    (command_code_parity) ? generate_parity(Command::code + 1,
						    ODD_PARITY) :
	    generate_parity(Command::code, ODD_PARITY);
	uint32_t address_parity =
	    (command_address_parity) ? generate_parity(address + 1,
						       ODD_PARITY) :
	    generate_parity(address, ODD_PARITY);

	if (psl_afu_command
	    (afu_event, new_tag, tag_parity, Command::code, code_parity,
	     address, address_parity, command_size, abort,
	     context) != PSL_SUCCESS)
		error_msg("OtherCommand: failed to send command");

	// TODO may not need
	if (afu_event->command_valid)
		info_msg("OtherCommand: command sent");
...
void LoadCommand::send_command(AFU_EVENT * afu_event, uint32_t new_tag,
			       uint64_t address, uint16_t command_size,
			       uint8_t abort, uint16_t context)
{
	if (Command::state != IDLE)
	...
	info_msg("LoadCommand: command sent");
...
void LoadCommand::process_response(AFU_EVENT * afu_event, uint8_t * cache_line)
{
	if (Command::state == WAITING_DATA) {
		if (afu_event->buffer_write == 1
			...
			Command::state = WAITING_RESPONSE;
			info_msg
			    ("LoadCommand: received buffer write in Waiting Data");
			...
			error_msg
			    ("LoadCommand: input not recognized, state: %d",
			     WAITING_DATA);
		}
	} else if (Command::state == WAITING_RESPONSE) {
		if (afu_event->buffer_write == 1
...
void LoadCommand::process_buffer_write(AFU_EVENT * afu_event,
				       uint8_t * cache_line)
{
	Command::state = WAITING_RESPONSE;
	...
	for (int i = 0; i < 4; ++i)
		info_msg("LoadCommand: received 0x%x for buffer write %d",
			 cache_line[i], i);
...
void StoreCommand::send_command(AFU_EVENT * afu_event, uint32_t new_tag,
				uint64_t address, uint16_t command_size,
				uint8_t abort, uint16_t context)
{
	if (Command::state != IDLE)
	...
	info_msg("StoreCommand: command sent");
...
void StoreCommand::process_buffer_read(AFU_EVENT * afu_event,
				       uint8_t * cache_line)
{
	uint8_t parity[2];
	generate_cl_parity(cache_line, parity);
	...
	for (int i = 0; i < 4; ++i)
		info_msg("StoreCommand: sending 0x%x for buffer read %d",
			 cache_line[i], i);
