...
AFU::resolve_mmio_descriptor_event ()
{
	...
	if (mmio_read_parity)
	    parity = generate_parity (data + 1, 1);
	else
	    parity = generate_parity (data, 1);
...
AFU::resolve_mmio_event ()
{
	...
	if (afu_event.mmio_address == 0x4) {
		...
		data = (mmio_regs[2] & 0xFFFFFFFF00000000);
	    data |= (mmio_regs[2] >> 32);
	    ...
	    mmio_addr = afu_event.mmio_address - MACHINE_CONFIG_OFFSET;
	    if (afu_event.mmio_double) {
		if (afu_event.mmio_address & 0x1)	// address not even
		    error_msg
			("AFU: Reading double word with non-even word address");
		data = machine_controller.get_machine_config (mmio_addr);
		data <<= 32;
		data |= machine_controller.get_machine_config (mmio_addr + 1);
		debug_msg ("AFU: Read MMIO data 64 address 0x%x, data %016lx",
			   afu_event.mmio_address, data);
	    }
	    else {
		data = machine_controller.get_machine_config (mmio_addr);

		// duplicate data in both top and bottom 32 bits
		data = (data & 0xFFFFFFFF) | (data << 32);

		debug_msg ("AFU: Read MMIO data 32 address 0x%x, data %08lx",
			   afu_event.mmio_address, data);
	...
	if (afu_event.mmio_address == 0x0) {
	    ...
	    mmio_addr = afu_event.mmio_address - MACHINE_CONFIG_OFFSET;
	    if (afu_event.mmio_double == 1) {
		machine_controller.change_machine_config (mmio_addr + 1,
							  afu_event.mmio_wdata
							  & 0xFFFFFFFF);
		machine_controller.change_machine_config (mmio_addr,
							  (afu_event.mmio_wdata
							   >> 32) &
							  0xFFFFFFFF);
		debug_msg
		...
		machine_controller.
		    change_machine_config (afu_event.mmio_address -
					   MACHINE_CONFIG_OFFSET,
					   (afu_event.mmio_wdata &
					    0xFFFFFFFF));

		debug_msg ("AFU: Write MMIO data 32 address 0x%x, data %08lx",
			   afu_event.mmio_address, afu_event.mmio_wdata);
	    }
	}
