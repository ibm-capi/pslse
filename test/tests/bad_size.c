...
/* Description : bad_size.c
 *
 * This test performs memcopy but intentionally tries to do the write with
 * an invalid, not power of two, size.
 */
...
int main(int argc, char *argv[])
{
	...
	// Generate bad command size
	do {
		size = rand() % CACHELINE_BYTES;
	} while (((size - 1) & size) == 0);
	// Use AFU Machine 1 to write the data to the second cacheline
	if ((response = config_enable_and_run_machine(afu_h, &machine, 1, 0, PSL_COMMAND_WRITE_NA, size, 0, 0, (uint64_t)cacheline1, CACHELINE_BYTES, DEDICATED)) < 0)
	{
		printf("FAILED:config_enable_and_run_machine");
		goto done;
	}
