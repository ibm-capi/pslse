...
class MachineController::Machine {
	private:
		...
		/* machine randomly selects a number (between min_delay and max_delay) of cycles
		 * to delay the generation of next command in the enable_always mode, initial 
		 * delay is always 0 */
		int delay;
		...
		void record_response(bool error_state, uint16_t cycle,
				uint8_t response_code);

		/* sets reponse code to FF to indicate that response_status, reponse_timestamp are currently invalid */
		void clear_response();
	...
	public:
		...
		/* configures the machine when AFU receives an MMIO write, only modifies the 
		 * config space, machine reads the config right before the command is sent */
		void change_machine_config(uint32_t offset, uint32_t data);

		/* depending on the offset, returns the configuration of machine when AFU receives an MMIO read */
		...
		/* read config and send new command if conditions are satisfied, returns true if a command is sent */
		...
		/* returns true if the current command is completed, i.e. in delayed phase */
