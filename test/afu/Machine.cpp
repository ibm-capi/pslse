...
void MachineController::Machine::reset()
{
	delay = 0;
	command = NULL;
...
void MachineController::Machine::read_machine_config()
{
	...
	bool command_address_parity =
	    (get_command_address_parity())? true : false;
	...
	info_msg("command address parity %d", get_command_address_parity());
	info_msg("command code parity %d", get_command_code_parity());
	info_msg("command tag parity %d", get_command_tag_parity());
	info_msg("buffer read parity %d", get_buffer_read_parity());
...
void MachineController::Machine::record_command(bool error_state,
						uint16_t cycle)
{
...
void MachineController::Machine::record_response(bool error_state,
						 uint16_t cycle,
						 uint8_t response_code)
{
	...
	config[1] =
	    (config[1] & 0xFFFFFF00FFFFFFFF) | ((uint64_t) response_code << 32);
}
							 ...
							 uint32_t data)
{
	...
	// lower 12 bits read only
		...
		if (offset % 2 == 1)
			config[offset / 2] =
			    (config[offset / 2] & 0xFFFFFFFF00000000) | data;
		else
			config[offset / 2] =
...
uint32_t MachineController::Machine::get_machine_config(uint32_t offset)
{
	...
	if (offset % 2 == 1)
		...
		return (uint32_t) ((config[offset / 2] & 0xFFFFFFFF00000000) >>
				   32);
}
						     ...
						     uint16_t cycle)
{
	...
	if ((!command || command->is_completed()) && delay == 0) {
		...
		uint64_t address_offset =
		    (rand() %
		     (memory_size - (command_size - 1))) & ~(command_size - 1);
...
void MachineController::Machine::process_response(AFU_EVENT * afu_event,
						  bool error_state,
						  uint16_t cycle)
{
...
void MachineController::Machine::process_buffer_write(AFU_EVENT * afu_event)
{
	if (command->get_tag() != afu_event->buffer_write_tag)
		error_msg
		    ("Machine: buffer_write_tag mismatches tag in machine");

	command->process_response(afu_event, cache_line);
}

void MachineController::Machine::process_buffer_read(AFU_EVENT * afu_event)
{
...
void MachineController::Machine::disable_once()
{
	config[0] &= ~0x4000000000000000;
}

void MachineController::Machine::disable()
{
